...
bool CollectionRegistry::showAllItems()	// Display all items in registry
{
...
void CollectionRegistry::sort(bool asc)	// Sort registry by title
{
...
void CollectionRegistry::emptyReg()	// Empty register
{
	BaseCollectionItem* pBase;
...
void CollectionRegistry::saveReg()	// Save register to file
{
	fstream file;
...
void CollectionRegistry::loadReg()	// Load register from file
{
	fstream file;
	string	itemType, title, author, genre, latinTitle, family;
	unsigned releaseYear, nrOfPages;
	...
	while (file.good())	// Not EOF or fail
	{
		...
		if (textLine == "%%%%%")	// Item separator line
		{
			...
			try {
				...
				std::cerr << "\nCould not read numerical value from file" << '\n';
			}
		}
	}
